...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- DOM Elements ---
    // (No changes to element selectors)
    ...
    // --- IMAGE PATHS (Confirm these match files in your ROOT folder) ---
    const IMAGE_PATHS = {
    ...
    // --- Content Data Store (Triple Checked Ingredients/Steps Arrays) ---
    const contentData = {
         en: {
             ...
             cookies: {
                classic: {
                      ...
                      // *** Classic Ingredients Array Starts Here ***
                      ingredients: [
                          { emoji: '🧈', text: 'Brown Butter: 1 cup (227g), <span class="critical">COOLED but LIQUID</span> <span class="critical">(vital!)</span>' }, // Added critical class to (vital!)
                      ...
                      ], // *** Classic Ingredients Array ENDS Here ***
                      // *** Classic Steps Array Starts Here ***
                       steps: [
                  ...
                  thick: {
                       ...
                       // *** Thick Ingredients Array Starts Here ***
                       ingredients: [
                       ...
                       ], // *** Thick Ingredients Array ENDS Here ***
                       // *** Thick Steps Array Starts Here ***
                       steps: [
                   ...
                   thin: {
                       ...
                       otherNotes: "Go light on flour (<span class='highlight'>~2 1/4 to 2 1/2 cups / 280-300g</span>). <span class='critical'>ZERO Baking Powder!</span> More <span class=\"highlight\">WHITE sugar</span> = crisp factor up! <span class='highlight'>Optional: 1-2 Tbsp Milk</span> for paper-thin results.",
                        // *** Thin Ingredients Array Starts Here ***
                       ingredients: [
                            ...
                            { emoji: '🧂', text: 'Salt: 1 tsp Kosher (or 1/2 tsp table)'},
                            { emoji: '🥛', text: 'Optional Flavor Bomb: 3-4 Tbsp (20-25g) Toasted Milk Powder'}
                       ], // *** Thin Ingredients Array ENDS Here ***
                        // *** Thin Steps Array Starts Here ***
                       steps: [
                       ...
                       ], // *** Thin Steps Array ENDS Here ***
                        customScienceNote: "Why it works: Warm liquid butter melts FAST in the oven = maximum SPREAD! Less flour = less structure holding it back. High white sugar caramelizes for SNAP and helps spread too. No baking powder = no extra lift fighting the spread. It’s physics for flat, crispy deliciousness!"
                    } // End Thin Cookie Object
              }, // End Cookies Object
              tips: [ // ** English Tips Array START (Checked commas/structure) **
                  ...
                  { key: 'tip9', emoji: '❄️', text: 'Freeze Like a Pro: Scoop dough balls (~30-45g each) onto a tray, freeze solid, then bag \'em. Bake straight from frozen! Add 1-2 mins baking time (maybe lower temp slightly ~350F/175C). Fresh cookies ANYTIME! YES!' },
                  { key: 'sci1', emoji: '🔥', text: 'Brown Butter = Flavor Gold: It\'s toasted milk solids & nutty goodness!' },
                  { key: 'sci2', emoji: '🥛', text: 'Toasted Milk Powder = Extra Credit Flavor: More nutty, caramelly notes? Sign me up.' } // Last item, no comma
             ] // ** English Tips Array END **
         }, // End 'en' object
         ar: { // ** Start Arabic Object **
              ...
              cookies: { // ** Arabic Cookie Objects START **
                  classic: { // ** Arabic Classic START **
              ...
              }, // ** Arabic Cookies Object END **
              tips: [ // ** Arabic Tips START **
              ...
              ] // ** Arabic Tips END - NO COMMA **
         } // ** END 'ar' Object - NO COMMA **
    }; // ** END contentData Object - VERY CAREFUL: Semicolon HERE, Ensure NO commas after previous closing braces **
        ...
        elements.forEach(el => {
            ...
            if ((key === 'keyDifferencesTitle' || key === 'recipeTitlePrefix') && currentCookieType && langData.cookies && langData.cookies[currentCookieType]) {
                ...
                if(key === 'keyDifferencesTitle'){
                    let baseTitle = (langData.keyDifferencesTitle || '').split('!')[0] + "!"; // Get base title text robustly
                    text = `${baseTitle} <span class='dynamic-cookie-name'>${cookieName}</span>`; // Append name!
                } else { // recipeTitlePrefix
                    text = `${langData.recipeTitlePrefix || 'Let\'s bake'} ${cookieName}!`;
                }
            } else if (key === 'keyDifferencesTitle') {
                 // Ensure reset to generic if no cookie type selected
                text = langData.keyDifferencesTitle || '🔑 Key Differences Breakdown!';
           }
    ...
    function updateRecipeView() {
         ...
         omarFavBubble.style.display = 'inline-block'; // Make sure it can transition
         requestAnimationFrame(() => { // Delay toggle slightly
        ...
        if (!currentCookieType || !langData.cookies || !langData.cookies[currentCookieType]) {
             ...
             updateTextContent(); // Ensure titles reset to generic
             return;
         }
         ...
         // Title update is handled by updateTextContent now
         ...
         // --- Integrate Science Note Directly Into Recipe ---
         let scienceHtml = '';
         ...
         // Added the science note at the end of the recipe container innerHTML
         ...
         if (showEasterEgg) {
            easterEggContainer.style.display = 'block';
             ...
             if (easterEggContainer.style.display !== 'none') { // Only set timeout if potentially visible
      ...
      function switchLanguage(lang) {
          ...
          updateTextContent(); // Update text content first
          updateRecipeView(); // THEN update based on new language and current cookie type
      }
      ...
      typeSelectorButtons.forEach(button => {
          button.addEventListener('click', (e) => {
               ...
               if (currentCookieType !== clickedType) { // Only update if type *changes*
                   typeSelectorButtons.forEach(btn => btn.classList.remove('active'));
                   button.classList.add('active');
                   currentCookieType = clickedType;
                   updateTextContent(); // Update text, including titles
                   updateRecipeView(); // Update view, sections, images
               }
           });
      });

     // --- Initial Setup ---
     switchLanguage(currentLanguage); // Set text, hide sections based on no initial cookie type
     // Keep header hidden initially
     cookieImageHeader.classList.remove('visible');
