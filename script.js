document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- DATA (Should be correct from previous iteration) ---
    const langData = {
        // --- English ---
        en: {
        ...
        // --- Arabic --- (Mirroring English data structure and changes)
        ar: {
    ...
    function updateLanguage(lang) {
        ...
        document.querySelectorAll('[data-lang-key]').forEach(el => {
            const key = el.dataset.langKey;
             if (key === 'keyDifferencesTitleBase') {
                 ...
                 el.firstChild.nodeValue = baseText;
             } else if (texts[key]) {
        ...
        if (selectedCookieType) {
            // Refresh texts in dynamic sections
            displayKeyDifferences(selectedCookieType); // Updates title and content text
             displayRecipe(selectedCookieType); // Regenerates recipe with correct language
                                               // (and will re-embed correct language unit toggles)
        } else {
            showPlaceholder(); // Update placeholder text
            // Ensure comparison image alt text is updated on lang change even if placeholder shown
             selectedCookieImage.alt = "Comparison of classic, thick, and thin cookies"; // Generic alt ok here
        }
        displayTips();
    }
    ...
    function createUnitTogglesHTML() {
        ...
        // --- Render Off-Screen Approach to get outerHTML with correct states ---
        toggleWrapper.style.position = 'absolute';
        toggleWrapper.style.visibility = 'hidden'; // Better than left -9999px
        toggleWrapper.style.zIndex = '-1';
        document.body.appendChild(toggleWrapper);
        // Apply current language/unit states before getting HTML
    ...
    function handleUnitChangeDelegation(event) {
        ...
        if (oldUnit !== currentUnit && selectedCookieType) {
            ...
            if (ingredientList) {
             ...
             const toggleWrapper = recipeDetailsContainer.querySelector('.unit-toggle-wrapper');
             if (toggleWrapper) updateUnitButtonActiveStates(toggleWrapper); // Still update buttons
        }
    }
    ...
    function displayRecipeContent(type) {
        ...
        const unitTogglesHtml = createUnitTogglesHTML(); // Create toggles HTML

        let contentHtml = `<div class="recipe-content-area">`;
        contentHtml += `<h3>${recipe.title}</h3>`;          // Title
        contentHtml += unitTogglesHtml;                     // Toggles embedded
        ...
        contentHtml += generateIngredientsHTML(type);       // Ingredients List
        ...
        if (recipe.scienceNote) {
            contentHtml += `<div class="science-note"><h4><span class="emoji">🔬</span> ${texts.scienceNoteTitle}</h4><p>${recipe.scienceNote}</p></div>`; // Science Note
        }
    ...
    function displayRecipe(type) {
        ...
        recipeDetailsContainer.innerHTML = ''; // Clear

        const recipeContentHtml = displayRecipeContent(type);
        recipeDetailsContainer.innerHTML = recipeContentHtml; // Add content (incl. toggles)

        const theme = langData[currentLang].recipes[type]?.theme || '';
        recipeDetailsContainer.className = `recipe-container ${theme}`;

        const isThick = (type === 'thick'); // Fave check remains on thick
        ...
        // Add event listener for unit changes WITHIN the newly rendered recipe container
    ...
    function showPlaceholder() {
         ...
         recipeDetailsContainer.removeEventListener('click', handleUnitChangeDelegation); // Remove listener
         ...
         // ** REMOVE class that makes image small **
         selectedCookieImage.classList.remove(IMAGE_CLASS_SELECTED);
         // --- END SET COMPARISON IMAGE ---
    ...
    function displayKeyDifferences(type) {
        ...
        if (nameSpan) nameSpan.textContent = diffs.name || 'Cookie';
    ...
    function handleCookieTypeSelect(event) {
        ...
        if (selectedCookieType === type) return; // No change if same type selected

        selectedCookieType = type; // Update state FIRST

        cookieTypeButtons.forEach(btn => btn.classList.remove('active'));
        button.classList.add('active');

        // --- SET SELECTED IMAGE (SMALL) ---
        ...
        // ** ADD class to make image small **
        selectedCookieImage.classList.add(IMAGE_CLASS_SELECTED);
         // --- END SET SELECTED IMAGE ---

        displayKeyDifferences(type);
        displayRecipe(type); // Renders recipe, attaches unit listener
    }
    ...
    function initialize() {
        ...
        // Initial State Setup (Show placeholder logic handles comparison image size)
        updateLanguage(DEFAULT_LANG); // Sets language, shows placeholder (incl. large comparison image)
        displayTips();
        ...
        // Initial image set within showPlaceholder, called by updateLanguage
