document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateLanguage(lang) {
        ...
        if(butterAmountInput) { // Ensure placeholder updates with language if necessary (e.g., numeric format or text part)
            // For this specific input, the placeholder is numeric, but good practice if it contained text
            butterAmountInput.placeholder = STANDARD_BUTTER_GRAMS.toString(); // Explicitly make it a string
        }
        ...
        displayTips(); // This will re-render tips in the new language
    }
    ...
    function createUnitTogglesHTML() {
        ...
        // Temporarily append to body to ensure styles are computed for outerHTML, then remove
    ...
    function handleUnitChangeDelegation(event) {
        ...
        } else if (oldUnit === currentUnit) { // Still update active state if same unit clicked (visual refresh)
             const toggleWrapper = recipeDetailsContainer.querySelector('.unit-toggle-wrapper');
             if (toggleWrapper) updateUnitButtonActiveStates(toggleWrapper);
        }
    }

    // --- VASTLY IMPROVED generateIngredientsHTML ---
    function generateIngredientsHTML(type) {
        const texts = langData[currentLang];
        const recipe = texts.recipes[type];
        if (!recipe?.ingredients) return '';

        // Determine which field in the ingredient object holds the metric string based on language
        // English uses 'metric', Arabic uses 'grams' for their primary metric string.
        const unitSystemKeyForMetric = (currentLang === 'ar') ? 'grams' : 'metric';
        // unitKey determines which measurement string to display (imperial/metric)
        ...
        recipe.ingredients.forEach(ing => {
            ...
            // unitSystemKeyForMetric helps check if the displayed unit IS a metric one
            if (unitKey === unitSystemKeyForMetric && currentScaleFactor !== 1) {
                ...
                if (ing.key === 'butter') {
                    ...
                    // Regex to find "STANDARD_BUTTER_GRAMS" followed by the gramMarker
                    ...
                    if (originalButterGramStringRegex.test(measurement)) {
                        ...
                        // Fallback: if the literal STANDARD_BUTTER_GRAMS isn't found (e.g., "About 226g...")
                        // try to replace the first found "NUMBER<gramMarker>" pattern.
                    ...
                    // For other ingredients, try to find "NUMBER-NUMBER<gramMarker>" first
                    const rangeRegex = new RegExp(`(\\d+)\\s*-\\s*(\\d+)\\s*${gramMarker}`);
                    const rangeMatch = measurement.match(rangeRegex);

                    if (rangeMatch && rangeMatch[1] && rangeMatch[2]) { // e.g. "15-20g"
                        ...
                        // If not a range, try to find a single "NUMBER<gramMarker>"
                        // This will also catch things like "(~100g)" or "approx 100g"
                        const singleGramRegex = new RegExp(`(\\d+(\\.\\d+)?)\\s*${gramMarker}`);
                        const singleMatch = measurement.match(singleGramRegex);
                        if (singleMatch && singleMatch[1]) { // singleMatch[0] is "100g", singleMatch[1] is "100"
                        ...
                        // If no specific gram amount is found with the marker (e.g. "2 large eggs"), it remains unscaled.
                    }
                }
            }
    ...
    function displayRecipeContent(type) {
        ...
        contentHtml += unitTogglesHtml; // Add unit toggles here
        contentHtml += `<h4 class="list-header" data-lang-key="ingredientsTitle">${texts.ingredientsTitle}</h4><ul class="ingredient-list">`;
        contentHtml += generateIngredientsHTML(type); // This now uses the advanced scaling
        contentHtml += '</ul>';
    ...
    function displayRecipe(type) {
        ...
        if (isThick && (!stuffedCookieImage.src || !stuffedCookieImage.src.endsWith(IMAGE_PATHS.stuffed))) { // Avoid re-setting src if already correct
        ...
        // Re-attach listeners to the new content if necessary (delegation preferred)
        // Cleanup existing listener specifically for unit changes IF it was directly on recipeDetailsContainer
        recipeDetailsContainer.removeEventListener('click', handleUnitChangeDelegation); // Defensive removal
        recipeDetailsContainer.addEventListener('click', handleUnitChangeDelegation); // Add it for the current recipe
    ...
    function showPlaceholder() {
         ...
         // Fix: Check if recipeDetailsContainer exists before trying to remove listener from it
         ...
         // Update placeholder text according to current language
         const placeholderDiv = recipeDetailsContainer.querySelector('.placeholder');
         if (placeholderDiv) { // Check if placeholder div exists before setting its innerHTML
            placeholderDiv.innerHTML = langData[currentLang].placeholderSelect;
         }

        if (selectedCookieImage.src !== IMAGE_PATHS.comparison){ // Avoid re-setting src if already correct
    ...
    function displayKeyDifferences(type) {
         ...
         // Update diff point titles based on language
         const headers = { 
    ...
    function displayTips() {
        ...
        if(tipBoxTitleElement && texts.tipsTitle) { 
             // Ensure emoji are part of the dynamic text from langData if they vary by lang, or hardcode if static
    ...
    function handleCookieTypeSelect(event) {
        ...
        if (selectedCookieType === type && selectedCookieImage.classList.contains(IMAGE_CLASS_SELECTED)) {
             // If same type is clicked and image is already specific, do nothing to avoid unnecessary re-renders
             // Or, you might want to re-animate or simply ensure active class is correct. For now, just return.
            return;
        }
        ...
        if (selectedCookieImage.src !== IMAGE_PATHS[type]){ // Avoid re-setting src if already correct
        ...
        displayRecipe(type); // This will use the currentScaleFactor
    }
    ...
    function initialize() {
        ...
        if (keyDiffTitleH3) {
            const baseTitleKey = 'keyDifferencesTitleBase';
            const initialCookieName = 'Cookie'; // A generic placeholder
        ...
        if (butterAmountInput) {
             butterAmountInput.value = STANDARD_BUTTER_GRAMS;
             butterAmountInput.placeholder = STANDARD_BUTTER_GRAMS.toString(); // Initial placeholder
        }
        
        updateLanguage(DEFAULT_LANG); // This will call showPlaceholder internally as selectedCookieType is null
        ...
        if (butterAmountInput) {
             ...
             butterAmountInput.addEventListener('change', handleScaleUpdate); // Also update if they tab out or lose focus after changing
        }
