...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const IMAGE_PATHS = {
        ...
        // comparison: '3_cookie_types.jpg', // This key seems unused now
        stuffed: 'stuffed_cookie.webp'      // UPDATE if needed
    };

    // --- Content Data Store (English & Arabic Translations) ---
    const contentData = {
    ...
    // --- Functions ---
         ...
         elements.forEach(el => {
             ...
             // Handle dynamic title injection
              if (key === 'keyDifferencesTitle' && currentCookieType && langData.cookies && langData.cookies[currentCookieType]) {
                 // Append cookie name to the base translated title
                 const baseTitle = langData.keyDifferencesTitle || 'Key Differences!'; // Get base title
                 const cookieName = langData.cookies[currentCookieType].name || currentCookieType;
                 text = `${baseTitle} <span class='dynamic-cookie-name'>${cookieName}!</span>`;
              } else if (key === 'keyDifferencesTitle') {
                 // Just use the base title if no cookie selected
                 text = langData.keyDifferencesTitle || 'Key Differences!';
              } else if (key === 'recipeTitlePrefix' && currentCookieType && langData.cookies && langData.cookies[currentCookieType]) {
                 // Append cookie name to the recipe prefix
                 text = `${langData.recipeTitlePrefix || 'Recipe:'} ${langData.cookies[currentCookieType].name || currentCookieType}!`;
              } else if (key === 'easterEggPistachioLink' && langData.pistachioLink) { // Example for link handling
                 // If you had a specific key for the link itself
                 el.href = langData.pistachioLink;
                 // Text content might be separate or part of the main key
                 text = langData[key] || 'Link'; // Assuming text is handled by key
              }

             // Only update the element's HTML if the text content has actually changed
             if (el.innerHTML !== text) {
                 el.innerHTML = text;
             }
         });

          // Update document title (strip HTML tags for safety)
          document.title = langData.mainTitle ? langData.mainTitle.replace(/<[^>]*>?/gm, '') : "Omar's Cookie Guide!";

         // Update Tips List - Clear and rebuild is often simplest
          tipsListContainer.innerHTML = '';
          if (langData.tips) {
             langData.tips.forEach(tip => {
                  const li = document.createElement('li');
                  li.dataset.emoji = tip.emoji || '💡'; // Add default emoji
                  li.innerHTML = tip.text; // Use innerHTML for potential formatting in tips
     ...
     function updateRecipeView() {
          ...
          // --- Omar's Favorite Bubble ---
          // Use CSS classes for transitions if possible, rather than direct style manipulation
          if(omarFavBubble) { // Check if element exists
         ...
         if (!currentCookieType || !langData.cookies || !langData.cookies[currentCookieType]) {
             ...
             if(keyDifferencesContainer) keyDifferencesContainer.classList.add('visually-hidden'); // Hide diffs using CSS class
             if(cookieImageHeader) cookieImageHeader.classList.remove('visible');
             if(easterEggContainer) easterEggContainer.classList.remove('visible'); // Hide Easter Egg

             // Reset key difference title (remove dynamic name part)
             ...
             return; // Exit function, nothing more to display
         }

          // --- Valid Cookie Selected - Update View ---
          const recipe = langData.cookies[currentCookieType];
          if(!recipe) return; // Should not happen if currentCookieType is valid, but safety check

          // Set theme class on recipe container
         if(recipeDetailsContainer) recipeDetailsContainer.className = `recipe-container ${recipe.theme || ''}`;

         // Update Top Image
          if (selectedCookieImage && cookieImageHeader) {
             const imageKey = recipe.imageSrcKey;
             const imagePath = IMAGE_PATHS[imageKey] || ''; // Default to empty if key not found
             selectedCookieImage.src = imagePath;
             selectedCookieImage.alt = `Omar's fantastic ${recipe.name || currentCookieType}`;
             if (imagePath) { // Only show header if image path is valid
         ...
         if(keyDifferencesContainer) {
             keyDifferencesContainer.classList.remove('visually-hidden'); // Show diffs
             if(butterMethodDesc) butterMethodDesc.innerHTML = recipe.butterMethod || 'N/A';
             if(chillingMethodDesc) chillingMethodDesc.innerHTML = recipe.chillingMethod || 'N/A';
             if(otherNotesDesc) otherNotesDesc.innerHTML = recipe.otherNotes || 'N/A';
             // Key diff title name is updated in updateTextContent
         }
         ...
         if(recipeDetailsContainer) {
             ...
             if(recipe.ingredients && Array.isArray(recipe.ingredients)){
                 recipe.ingredients.forEach(ing => {
                     // Add a class based on keywords for potential specific styling/icons
                     let itemClass = '';
                     if (ing.text.toLowerCase().includes('butter')) itemClass = 'butter';
                     else if (ing.text.toLowerCase().includes('sugar')) itemClass = 'sugar';
                     else if (ing.text.toLowerCase().includes('flour')) itemClass = 'flour';
                     else if (ing.text.toLowerCase().includes('egg')) itemClass = 'eggs';
                     else if (ing.text.toLowerCase().includes('chocolate')) itemClass = 'choco';

                     ingredientsHtml += `<li class="${itemClass}" data-emoji="${ing.emoji || '🍪'}">${ing.text}</li>`;
                 });
             }
             ingredientsHtml += '</ul>';
             ...
             if(langData.howToToastMilkPowderTitle && langData.howToToastMilkPowder){
                 howToToastHtml = `<div class="how-to-toast"><h4>${langData.howToToastMilkPowderTitle}</h4><p>${langData.howToToastMilkPowder}</p></div>`;
             }
             ...
             if (recipe.customScienceNote && langData.scienceHeader) {
                 scienceHtml = `<div class="science-note"><h4>${langData.scienceHeader}</h4><p>${recipe.customScienceNote}</p></div>`;
             }

             recipeDetailsContainer.innerHTML = `<h3>${langData.recipeTitlePrefix || 'Recipe:'} ${recipe.name || currentCookieType}!</h3>${ingredientsHtml}${stepsHtml}${scienceHtml}`;
         }

          // --- Easter Egg Logic ---
          if(easterEggContainer) { // Check if element exists
             const showEasterEgg = (currentCookieType === 'thick');
             if (showEasterEgg) {
                 // Ensure image exists before trying to set src
                 if (stuffedCookieImage) {
                    stuffedCookieImage.src = IMAGE_PATHS.stuffed || ''; // Use || '' as fallback
                    stuffedCookieImage.alt = "Amazing stuffed cookie"; // Consider translating alt text too
                 }
                 easterEggContainer.style.display = 'block'; // Make it take space *before* transition
                 // Delay adding visible slightly to allow display:block to register
                 requestAnimationFrame(() => {
                    requestAnimationFrame(() => { // Double RAF for better reliability on some browsers
                       easterEggContainer.classList.add('visible');
                    });
                 });
             } else {
                 easterEggContainer.classList.remove('visible');
                 // Let CSS handle hiding via transitionend or rely on max-height/opacity transition
                 // No need for JS timeout if CSS handles the transition and eventual display:none or visibility:hidden
             }
          }
     }
      ...
      function switchLanguage(lang) {
          if (!contentData[lang]) {
              ...
              lang = 'en'; // Fallback to English if requested lang doesn't exist
          }
          currentLanguage = lang;
          body.dir = (lang === 'ar') ? 'rtl' : 'ltr';
          langButtons.forEach(btn => btn.classList.toggle('active', btn.dataset.lang === lang));
          updateTextContent(); // Update all static text based on new language
          updateRecipeView(); // Re-render the recipe/differences view with new language
      }
      ...
      langButtons.forEach(button => button.addEventListener('click', (e) => {
          ...
          if (lang && lang !== currentLanguage) { // Only switch if lang exists and is different
              switchLanguage(lang);
          }
      }));
      ...
      typeSelectorButtons.forEach(button => {
          button.addEventListener('click', (e) => {
               ...
               if (clickedType === currentCookieType) {
                   // Optional: Allow toggling off? Or do nothing if already active.
                   // console.log("Already selected:", clickedType);
                   return; // Keep it simple: do nothing if clicking the active one
               }
              ...
              updateTextContent(); // Update general text fields first (like titles that need the name)
              updateRecipeView(); // Then update the specific recipe view, differences, image etc.

              // Optional: Scroll to the recipe details smoothly
              if (recipeDetailsContainer) {
                  recipeDetailsContainer.scrollIntoView({ behavior: 'smooth', block: 'start', inline: 'nearest' });
              }
           });
      });
     ...
     switchLanguage(currentLanguage); // Set initial language & render placeholder text

     // Ensure sections that depend on selection are hidden initially
     if (keyDifferencesContainer) keyDifferencesContainer.classList.add('visually-hidden');
     if (cookieImageHeader) cookieImageHeader.classList.remove('visible');
     if (easterEggContainer) {
         easterEggContainer.classList.remove('visible');
         easterEggContainer.style.display = 'none'; // Ensure it doesn't take space initially
     }
     if(omarFavBubble) omarFavBubble.classList.remove('visible'); // Hide fav bubble


     // Fade in the page slightly after initial setup
     setTimeout(() => {
         body.classList.add('loaded');
         console.log("Page loaded and faded in.");
     }, 100); // Small delay
